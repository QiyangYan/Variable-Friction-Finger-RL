...
from gymnasium_robotics.envs.variable_friction_withFramework import (
    MujocoManipulateEnv,
)

# Ensure we get the path separator correct on windows
MANIPULATE_BLOCK_XML = os.path.join("/content/gymnasium_robotics/envs/assets/variable_friction", "IHM.xml")
...
class MujocoHandBlockEnv(MujocoManipulateEnv, EzPickle):
    ...
    * `HandManipulateBlockFull-v1`: Random target rotation for all axes of the block. Random target position.
    ...
    The action space is a `Box(-1.0, 1.0, (3,), float32)`. The control actions are absolute angular positions of the actuated joints (non-coupled). The input of the control actions is set to a range between -1 and 1 by scaling the actual actuator angle ranges.
