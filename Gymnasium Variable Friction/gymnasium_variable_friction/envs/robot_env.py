...
class BaseRobotEnv(GoalEnv):
    ...
    def step(self, action):
        ...
        if np.array(action).shape != self.action_space.shape:
            # print(np.array(action).shape,self.action_space.shape)
            raise ValueError("Action dimension mismatch")

        # action_friction = action[1]
        # action = np.clip(action, self.action_space.low, self.action_space.high)
        # action[1] = action_friction
    ...
    def reset(
        ...
        *,
        ...
        self.goal = self._get_contact_point(self.goal)
        obs["desired_goal"] = self.goal
        # print("goal after reset:",self.goal)
    ...
    def _get_contact_point(self):
        '''
        get the right most and left most coordinate as the contact point
        think: use the right/left most coordinate or largest/smallest gradient of the line between corner and motor
        '''
...
class MujocoRobotEnv(BaseRobotEnv):
    ...
    def _mujoco_step(self, action):
        # print("_mujoco_step",self.n_substeps)
        self._mujoco.mj_step(self.model, self.data, nstep=self.n_substeps)

    def adjust_torque(self,lower):
        if lower:
            self.data.ctrl[1] = self.data.ctrl[1] / 2
        else:
            self.data.ctrl[1] = 1
    ...
    def check_action_complete(self):
        '''
        self.data.ctrl[4] is stand
        [0],[1] is motor [1] is torque control
        self.data.qpos[0] is stand
        [1],[3] is motor [1] is left finger
        '''

        self.motor_diff = self.data.ctrl[0] - self.data.qpos[1]
        friction_diff1 = self.data.ctrl[2]/100 - self.data.qpos[2]
        friction_diff2 = self.data.ctrl[3]/100 - self.data.qpos[4]

        if abs(friction_diff1) < 0.0001 and abs(friction_diff2) < 0.0001 and abs(self.motor_diff) < 0.04:
            return True
        else:
            # print("check action complete",abs(friction_diff1),abs(friction_diff2), abs(self.motor_diff))
            # print(self.data.ctrl[2:4],self.data.qpos[2],self.data.qpos[4])
            # print(self.data.ctrl[0],self.data.qpos[1])
            # print("friction_diff1:", friction_diff1, "friction_diff2:", friction_diff2, "motor_diff:", motor_diff)
            return False

    def slide_complete(self):
        return self.successSlide
...
class MujocoPyRobotEnv(BaseRobotEnv):
    ...
    def render(self):
        ...
        if self.render_mode in {
            ...
            viewer = self.render("human")._viewer
