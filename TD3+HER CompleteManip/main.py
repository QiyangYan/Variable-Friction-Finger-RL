...
agent = Agent(n_states=state_shape,
              ...
              k_future=k_future,
              env=dc(env))
# agent.load_weights("/Users/qiyangyan/Desktop/TD3+HER/Pre-trained models/VariableFriction-v0_after46epoch_smallerSuccessRegion.pth")
    ...
    for ep in range(10):
        ...
        for t in range(50):
            ...
            if action[1] != last_f:
                ...
                complete = change_friction(np.array(friction_action1), env, 0.001)
                if complete == False:
                    # print("terminated during friction changing")
                    break
                complete = change_friction(np.array(friction_action2), env, 0.0003)
                if complete == False:
                    # print("terminated during friction changing")
                    break
...
def modify_action(state):
    angle = state["observation"][0]
...
def action_preprocess(action):
    ...
    #     friction_state = -1
    # elif -0.33 < friction_state < 0.33 and env.slide_complete():
    #     friction_state = 0
    # else:
    #     print(friction_state)
    #     assert(friction_state,2) # 随便写个东西来报错
    '''only sliding'''
    if env.slide_complete():
...
if Train:
        ...
        for cycle in range(0, MAX_CYCLES):
            ...
            for episode in range(MAX_EPISODES):
                ...
                for t in range(10):
                    ...
                    if terminated:
                        # print(terminated)
                        action = modify_action(next_env_dict)
                        break
            ...
            print("-------------------------------------------------------------")
            print(mb[0]["desired_goal"])
            print("-------------------------------------------------------------")
            print(mb[1]["desired_goal"])
            print("-------------------------------------------------------------")
            # print("agent store")
            agent.store(mb)
            ...
            print("Time Taken for one train, one cycle:", time.time() - train_time, time.time() - start_time_t)
    ...
    address_list = [
        ...
        "/Users/qiyangyan/Desktop/TD3+HER/Pre-trained models/VariableFriction-v0_after30epoch_success0.5.pth"
    ...
    for address in address_list:
        ...
        for index in range(max_episode):
            ...
            print("epoch: ",index,success_rate)
